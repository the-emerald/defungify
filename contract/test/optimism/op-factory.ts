...
describe("Optimistic Defungify Factory", function () {
    let accounts: any;
    let erc20Token: any;
    ...
    beforeEach(async function () {
        ...
        const DefungifyFactory = await ethers.getContractFactory("OpDefungifyFactory");
    ...
    it("Should have consistent naming", async function () {
        await defungifyFactory.deployDf(erc20Token.address);
        const deployed = await ethers.getContractAt("OpDefungify", await defungifyFactory.deployedContracts(erc20Token.address));
    ...
    it("Should not be able to initialize twice", async function() {
        await defungifyFactory.deployDf(erc20Token.address);
        const deployed = await ethers.getContractAt("OpDefungify", await defungifyFactory.deployedContracts(erc20Token.address));
        expect(deployed.initialize(erc20Token.address))
            .to.be
            // @ts-ignore
